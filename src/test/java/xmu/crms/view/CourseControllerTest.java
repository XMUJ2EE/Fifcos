    ...
    public void createSeminarForCourse() throws Exception {
        mvc
                .perform(post("/course/{courseId}/seminar", 1)
                        .contentType(MediaType.APPLICATION_JSON_UTF8)
                        .content("{\"id\":29,\"name\": \"概要设计\", \"description\": \"模型层与数据库设计\", \"groupingMethod\": \"fixed\", \"startTime\": \"2017-10-10\", \"endTime\": \"2017-10-24\", \"proportions\": {\"report\": 50, \"presentation\": 50, \"c\": 20, \"b\": 60, \"a\": 20 }}".getBytes()))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.id").isNumber())
                .andDo(print());
    }
